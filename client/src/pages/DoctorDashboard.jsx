...
export default function DoctorDashboard() {
  ...
  const displayName = (me && me.fullname) || (JSON.parse(localStorage.getItem('user') || '{}').fullname) || ''
  ...
  return (
    <div className="relative min-h-screen bg-gradient-to-b from-agri-200 via-agri-100 to-agri-50 text-agri-900">
        ...
        <div className="max-w-6xl mx-auto px-4 py-3 flex items-center">
          ...
          <div className="flex items-center gap-3 ml-auto pr-0">
            <button onClick={() => setPanelOpen(true)} className="hidden sm:flex items-center gap-2 rounded-full border border-agri-400 bg-white hover:bg-agri-100 text-agri-800 px-3 py-1.5">
              ...
              <span className="font-medium truncate max-w-[12rem]">{displayName || 'Veterinary Doctor'}</span>
            </button>
      ...
      <main className="relative max-w-6xl mx-auto px-4 pt-20 pb-10">
        <h2 className="text-2xl font-bold">Veterinary Doctor Dashboard</h2>
        <p className="mt-2 text-agri-700">Manage animal health services and veterinary care</p>
        <div className="mt-20 grid gap-20 md:gap-40 md:grid-cols-3 place-items-center">
          <Card
            title="Consultations"
            description="Manage animal consultations and appointments"
            icon="🏥"
            ghost
            onClick={() => alert('Consultation management coming soon!')}
          />
          <Card
            title="Animal Records"
            description="Access and manage livestock health records"
            icon="📋"
            ghost
            onClick={() => alert('Animal records management coming soon!')}
          />
          <Card
            title="Emergency Care"
            description="Emergency veterinary services and support"
            icon="🚨"
            ghost
            onClick={() => alert('Emergency care services coming soon!')}
          />
        </div>
      </main>
        ...
        <div className="fixed inset-0 z-20">
          ...
          <div className="absolute right-4 top-24 w-96 bg-gradient-to-b from-agri-200 via-agri-100 to-agri-50 border border-agri-300 rounded-2xl shadow-2xl p-5 max-h-[80vh] overflow-y-auto">
            <div className="flex items-center justify-between">
              <h3 className="text-lg font-semibold text-agri-900">Veterinary Doctor Profile</h3>
            ...
            <div className="mt-4 space-y-3 text-sm">
              ...
              {!loadingMe && (
                <>
                  <div><span className="font-medium">Name:</span> {me?.fullname || JSON.parse(localStorage.getItem('user')||'{}')?.fullname || '-'}</div>
            ...
            <div className="mt-6 pt-4 border-t border-agri-300 space-y-2">
              <button onClick={goToEditProfile} className="w-full text-left px-3 py-2 rounded-lg bg-agri-600 text-white hover:bg-agri-700 transition-colors">
                Edit Profile
              </button>
              <button onClick={goToHistory} className="w-full text-left px-3 py-2 rounded-lg bg-agri-600 text-white hover:bg-agri-700 transition-colors">
                History
              </button>
              <button onClick={goToHome} className="w-full text-left px-3 py-2 rounded-lg bg-agri-600 text-white hover:bg-agri-700 transition-colors">
                Home
              </button>
            </div>
          </div>
